...
class HyperDHTLookup {
   ...
   * @param {string|Buffer} [opts.capability]
   */
  constructor (opts) {
    this._dht = opts.dht
    ...
    if (opts.capability && (typeof opts.capability !== 'string' || !Buffer.isBuffer(opts.capability))) {
      throw new Error('ERR_CAPABILITY_INALID')
    }
    this._requestOpts = {}
    if (opts.capability) {
      this._requestOpts.capability = typeof opts.capability === 'string'
        ? Buffer.from(opts.capability, 'hex')
        : opts.capability
    }
